...
export async function fetchProgress(speakerId: number): Promise<Record<string, number>> {
...
export async function fetchLatestPrompt(speakerId: number): Promise<Record<string, string>> {
  const res = await fetch(`/api/progress?speaker_id=${speakerId}`)
