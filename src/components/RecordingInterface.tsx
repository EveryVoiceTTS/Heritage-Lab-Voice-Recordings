...
export default function RecordingInterface({
  ...
  const batchRecordedCount = batchPrompts.filter((p) =>
    recordings.has(p.id),
  ).length;
  ...
  const handleSubmitBatch = useCallback(async () => {
    const zip = new JSZip();
    const ts = new Date();
    let humanReadableTs = ts
      .toLocaleString(undefined, { hour12: false }) //change to format dd-mm-yyyy_hh-mm-ss
      .replace(/\//g, "-")
      .replace(", ", "_")
      .replace(/:/g, "-");
    ...
    for (const p of batchPrompts) {
      ...
      if (rec) {
        ...
        try {
          ...
          console.warn(
            "Could not save recording to IndexedDB (quota may be exceeded):",
            e,
          );
        }
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.code === "Space" && playingId !== prompt.id) {
        ...
        if (hasCurrentRecording) {
          // Call rerecord methods if recording already exists
          handleReRecord();
        }
        handleRecord();
      }
      if (isRecording) {
        // Do not allow navigate actions while recording.
        return;
      ...
      <div className="flex-1 flex flex-col items-center overflow-y-auto min-h-0 p-6">
        <div className="bg-white rounded-2xl shadow-sm border border-cream-dark p-10 text-center max-w-md">
          ...
          <p className="text-charcoal-light mb-6">
            All recordings for this category have been saved locally.
          </p>
          <button
            onClick={() => {
              onSessionComplete();
        ...
        <div className="max-w-2xl w-full my-auto">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-cream-dark p-8 mb-8 flex items-center gap-6">
            <div className="flex-1 text-center">
              <p
                className="text-3xl leading-relaxed text-charcoal mb-3 font-medium font-syllabics"
                lang="iu"
              >
                {prompt.syllabics}
              </p>
              ...
              className={`
                  ...
                  ${
                    isRecording || !hasCurrentRecording
                      ? "cursor-not-allowed text-charcoal/50 bg-white/50"
          ...
          <div className="mb-6">
            <label
              htmlFor="prompt-note"
              className="flex items-center gap-1.5 text-xs font-medium text-charcoal-light mb-1.5"
            >
              <svg
            ...
            <textarea
              ...
              onChange={(e) =>
                setNotes((prev) => new Map(prev).set(prompt.id, e.target.value))
              }
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <div className="text-xs text-charcoal-light mt-1 flex flex-wrap gap-x-3 gap-y-1">
                <span>
                  <kbd className="px-1.5 py-0.5 bg-cream-dark rounded text-[10px] font-mono">
                    Space
                  </kbd>{" "}
                  record
                </span>
            ...
            <div className="flex gap-3">
              ...
              {(hasCurrentRecording || isCurrentSkipped) &&
